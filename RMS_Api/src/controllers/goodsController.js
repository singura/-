		...
		if(err){
		 ...
		 for (var i = 0; i < flng; i++) {
		 	...
		 	sqlValues += `(
							 ...
							 ,'${item.shorturl.startsWith('/') ? item.shorturl : '/' + item.shorturl}' /* thumb_path - VARCHAR(255)*/
							 ,'${item.shorturl.startsWith('/') ? item.shorturl : '/' + item.shorturl}' /* original_path - VARCHAR(255)*/
							 ,'' /* remark - TEXT*/
							 ,NOW() /* add_time - DATETIME*/
							)${splitChar}`;
		 };
