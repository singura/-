		...
		data() {
			return {
				...
				rules: {
					...
					market_price: [
						{ required: false, message: '请输入市场价格', trigger: 'blur' },
					],
					sell_price: [
						{ required: false, message: '请输入销售价格', trigger: 'blur' },
					],
					zhaiyao: [
		...
		methods: {
			...
			getData(){
				...
				this.ajax.get(url).then(res=>{
					...
					this.ruleForm.stock_quantity = this.ruleForm.stock_quantity.toString()
					this.ruleForm.market_price = this.ruleForm.market_price.toString();
					this.ruleForm.sell_price = this.ruleForm.sell_price.toString();	
				})
			},
			// 获取商品分类数据
			getcastlist(){
